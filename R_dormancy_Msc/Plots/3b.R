...
ggboxplot(df.merged9, x = "PYHIN1_aamutation_cna", y = "PYHIN1_log2expression",
  ...
  labs(x="PYHIN1 status",y="mRNA Expression (log2)",fill="GENEstatus_TP53status")+
...
ggboxplot(df.merged_gnaq2, x = "GNAQ_aamutation_cna", y = "GNAQ_log2expression",
          ...
          order = c("WT", "MUT","AMP","DEL")) + #添加jitter后，会自动隐藏异常值。
  stat_compare_means(comparisons = my_comparisons3)+
  labs(x="GNAQ status",y="mRNA Expression (log2)",fill="GENEstatus_TP53status")+
  guides(colour=guide_legend(title="GENE status (AA mutation and CNV)"))+
  scale_y_continuous(limits=c(0,20)) 
...
ggboxplot(df.merged_fgfr2_2, x = "FGFR2_aamutation_cna", y = "FGFR2_log2expression",
          ...
          order = c("WT", "MUT","AMP","DEL")) + #添加jitter后，会自动隐藏异常值。
  stat_compare_means(comparisons = my_comparisons3)+
  labs(x="FGFR2 status",y="mRNA Expression (log2)",fill="GENEstatus_TP53status")+
  guides(colour=guide_legend(title="GENE status (AA mutation and CNV)"))+
  scale_y_continuous(limits=c(0,20)) 
...
ggboxplot(df.merged_erbb3_2, x = "ERBB3_aamutation_cna", y = "ERBB3_log2expression",
          ...
          order = c("WT", "MUT","AMP","DEL")) + #添加jitter后，会自动隐藏异常值。
  stat_compare_means(comparisons = my_comparisons3)+
  labs(x="ERBB3 status",y="mRNA Expression (log2)",fill="GENEstatus_TP53status")+
  guides(colour=guide_legend(title="GENE status (AA mutation and CNV)"))+
  scale_y_continuous(limits=c(0,20)) 
...
ggboxplot(df.merged_plppr4_2, x = "PLPPR4_aamutation_cna", y = "PLPPR4_log2expression",
          ...
          order = c("WT", "MUT","AMP","DEL")) + #添加jitter后，会自动隐藏异常值。
  stat_compare_means(comparisons = my_comparisons3)+
  labs(x="PLPPR4 status",y="mRNA Expression (log2)",fill="GENEstatus_TP53status")+
  guides(colour=guide_legend(title="GENE status (AA mutation and CNV)"))+
  scale_y_continuous(limits=c(0,20)) 
